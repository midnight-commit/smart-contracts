...
import "./LiquidityBookStrategyBaseForSA.sol";
 ...
 */
contract LiquidityBookStrategyForSA is LiquidityBookStrategyBaseForSA {
    using SafeTransferLib for IERC20;
    using Math512Bits for uint256;

    uint256 internal constant SCALE_OFFSET = 128;
    uint256 internal constant PRECISION = 1e18;
    int256 internal constant ACTIVE_BIN = 0;
    ...
    int256 public immutable depositBin;
        ...
        YakStrategyV2(_strategySettings)
    {
        ...
        depositBin = _strategySettings.depositToken == _settings.tokenX ? int256(1) : int256(-1);
    ...
    function _deposit(address _account, uint256 _amount) internal {
        ...
        _mint(_account, sharesForDepositTokens(_amount));
        (uint256 amountX, uint256 amountY) = depositTokenToXY(_amount);
        _addLiquidityToSingleBin(amountX, amountY, depositBin, 0);
        emit Deposit(_account, _amount);
    }

    function sharesForDepositTokens(uint256 _amount) public view returns (uint256) {
        (, , uint256 activeId) = lbPair.getReservesAndId();
        (uint256 amountX, uint256 amountY) = depositTokenToXY(_amount);
        uint256 price = lbRouter.getPriceFromId(address(lbPair), uint24(activeId));
        uint256 amount = price.mulShiftRoundDown(amountX, SCALE_OFFSET) + amountY;
    ...
    function depositTokenToXY(uint256 _amount) internal view returns (uint256 amountX, uint256 amountY) {
        amountX = address(depositToken) == tokenX ? _amount : 0;
        amountY = address(depositToken) == tokenY ? _amount : 0;
    }

    function _addLiquidityToSingleBin(
        uint256 _amountX,
        uint256 _amountY,
        int256 _bin,
